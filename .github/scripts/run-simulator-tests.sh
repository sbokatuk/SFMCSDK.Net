...
# Builds the device test app against the packed SFMCSDK.Net package, installs it on an iOS
# simulator and runs its checks. The app prints its verdict to stdout; this script turns that
# into an exit code.
#
...
BUNDLE_ID="com.sbokatuk.sfmcnet.devicetests"
...
# affects. The sample job builds a real MAUI app against the same package and covers Release.
...
APP_PATH="$(find "${REPO_ROOT}/tests/SFMCSDK.Net.DeviceTests/bin/Debug/${TARGET_FRAMEWORK}/${SIMULATOR_RID}" \
...
if ! grep -q "SFMC_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    xcrun simctl spawn "${udid}" log show --last 2m --predicate "process == 'SFMCSDK.Net.DeviceTests'" \
        2>/dev/null | tail -100 | tee -a "${LOG_FILE}" || true
    echo "::error::SFMCSDK.Net simulator checks failed or timed out"
    exit 1
fi
