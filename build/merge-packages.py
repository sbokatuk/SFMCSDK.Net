...
The dependency group is copied rather than synthesised as an empty one. The façade's android and
ios groups carry its exact-pinned platform binding dependencies, and an empty group would tell
NuGet that a net10.0-ios consumer needs none of them - so SFMCSDK.Net would restore on net10
without SFMCSDK.Net.iOS underneath it, and the app would fail at link time with the native
framework missing.
